...
// Suponiendo que tienes 101 frames (de 0.gif a 100.gif) para la animación
...
export default function EquilibrioGame() {
  ...
  // Estado para el acelerómetro (y cooldown para evitar múltiples detecciones rápidas)
  const [cooldown, setCooldown] = useState(false);

  // Lógica del acelerómetro: si el total de aceleración supera el umbral, descuenta una vida
  useEffect(() => {
  ...
  // Animación del personaje: reproducir frames de 0 a 100 en un bucle
  useEffect(() => {
  ...
  // Callback para actualizar el tiempo del Cronometro
  const handleTimeChange = (seconds: number) => {
    setTime(seconds);
  };

  // Al finalizar el juego, se actualiza el logro en Supabase
  useEffect(() => {
    if (gameOver) {
      (async () => {
        const { data: { user } } = await supabase.auth.getUser();
        if (user) {
          // Para Equilibrio (categoría "4"), se acumula el tiempo (cumulative = true)
          updateAchievementProgress(user.id, '4', 1, time, true);
        }
      })();
    }
  }, [gameOver, time]);

  // Pantalla final: se muestra cuando se acaban las vidas
  if (gameOver) {
    return (
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.animationContainer}>
        <Image source={EQUILIBRIO_FRAMES[frameIndex]} style={styles.character} />
      </View>
    </SafeAreaView>
  );
}
